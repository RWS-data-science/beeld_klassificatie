...
schaal = 1 #pixel waarden tussen 0 en schaal
max_accuracy = 0.97 #opslaan vanaf

out = 0.5 #dropout
batch_train = 50 #batchsize
batch_test = 300 #batchsize test
...
#input
#reshape
w_conv1 <- weight_variable(shape(5L, 5L, 3L, 40L), 'w_conv2')
b_conv1 <- bias_variable(shape(40L), 'b_conv2')
#pool
#laag 4
w_conv2 <- weight_variable(shape = shape(5L, 5L, 40L, 64L), 'w_conv3')
b_conv2 <- bias_variable(shape = shape(64L), 'b_conv3')
#pool
#laag 5
w_conv3 <- weight_variable(shape = shape(5L, 5L, 64L, 64L), 'w_conv4')
b_conv3 <- bias_variable(shape = shape(64L), 'b_conv4')
#reshape
#laag 6
...
#convolutie laag 1
h_conv1 <- tf$nn$relu(conv2d(h_input, w_conv1) + b_conv1)
#poollaag 2
...
for (i in 1:20000) {
  ...
  if (i %% 100 == 0) {
    ...
    if(test_accuracy > max_accuracy  ){
      ...
      saveRDS(sess$run(w_conv1), file = 'db/netwerk/w_conv1.rds')
      saveRDS( sess$run(b_conv1), file = 'db/netwerk/b_conv1.rds' )
...
# 
# b_conv1 = readRDS(  file = 'db/netwerk/b_conv1.rds' )
# b_conv1 = tf$cast(b_conv1, tf$float32)
# 
